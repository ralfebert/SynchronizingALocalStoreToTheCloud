...
Abstract:
The view controller class for the master view, showing a list of posts.
*/

import UIKit
import CoreData

class MasterViewController: SpinnerViewController {
    @IBOutlet weak var composeItem: UIBarButtonItem!
    weak var detailViewController: DetailViewController?
    
    private lazy var dataProvider: PostProvider = {
        let appDelegate = UIApplication.shared.delegate as? AppDelegate
        let provider = PostProvider(with: appDelegate!.coreDataStack.persistentContainer,
...
extension MasterViewController: PostInteractionDelegate {
    /**
    ...
    /**
     ...
     - If the detailViewController is initialized and has a valid post, make sure the master view controller has the right selection.
     
     The master tableView selection is cleared when splitViewController is collapsed, so rely on the controller hierarchy to do the initialization.
     */
